...
import static org.pitest.mutationtest.report.xml.Tag.mutationDescription;
...
public class XMLReportListener implements MutationResultListener {
  ...
  private String makeMutationNode(final MutationResult mutation) {
    ...
    return makeNode(clean(details.getFilename()), sourceFile)
        ...
        + makeNode(clean(details.getDescription()), mutationDescription)
        + makeNode("" + details.getFirstIndex(), index)
        + makeNode(createCoveringTestsDesc(details.getTestsInOrder()), coveringTests)
        + makeNode(createAllKillingTestsDesc(mutation.getAllKillingTests()), killingTests);
  }
  ...
  private String createAllKillingTestsDesc(final Option< ArrayList<String> > allKillingTests) {
    if (allKillingTests.hasSome()) {
      final String temp = allKillingTests.value().toString();
