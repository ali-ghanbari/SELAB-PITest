...
public final class MutationStatusTestPair {
  ...
  private Option< ArrayList<String> > allKillingTests = Option.none();
  ...
  public void setAllKillingTests(Option< ArrayList<Description> > akt) {
    if (akt.hasSome()) {
       this.allKillingTests = Option.some(new ArrayList<String>());
       List<String> s = this.allKillingTests.value();
       for (Description desc : akt.value()) {
           s.add(desc.getQualifiedName());
       }
    }
  }

  public Option< ArrayList<String> > getAllKillingTests() {
    return this.allKillingTests;
  }
